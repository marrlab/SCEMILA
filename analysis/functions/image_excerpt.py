...
def plot(sc_df, cols=16, dpi=150, show_coordinates=True, show_scalebar=False, path_save=None, show_patient_class=False):
    ...
    fig = plt.figure(constrained_layout=False, figsize=(15, (rows)*scale_size), dpi=dpi)
    ...
    for gs_sub in gs:
        ...
        if show_scalebar and counter == 0:
            ax.plot((8, 8), (72 - 29, 72 + 29), c = 'k')                 # 10 mikrometer = 57,8px
...
def plot_multipatient(sc_df, cols=16, dpi=150, show_coordinates=True, show_scalebar=False, path_save=None, show_patient_class=False):

    im_ar_dict = {}
    for idx, r in sc_df.iterrows():
        if not r.name in im_ar_dict.keys():
            im_ar_dict[r.name] = np.load(os.path.join(os.path.dirname(r['im_path']), 'processed', 'stacked_images.npy'))
