...
def plot(df_row, reorder, path_save=None):
    ...
    ax.set_title('Prediction for patient {}'.format(
        os.path.basename(df_row.pat_path)), fontsize=12)

    # plot groundtruth label
    gt = df_row.gt_label
    gt_y = ypos[reorder.index(gt)]
    gt_x = df_row['mil_prediction_{}'.format(gt)]+0.05
    ax.scatter(gt_x, gt_y, marker='o', s=125, color='w',
               edgecolor='k', linewidth=1, label='Groundtruth')

    # plot predicted label
    pred = df_row.pred_lbl
    pred_y = ypos[reorder.index(pred)]
    pred_x = df_row['mil_prediction_{}'.format(pred)]+0.05
    ax.scatter(pred_x, pred_y, marker='x', s=125,
               color='k', linewidth=1, label='Prediction')

    leg = ax.legend(title='Patient label', title_fontsize=12,
                    fontsize=12, edgecolor='gainsboro', shadow=True)
