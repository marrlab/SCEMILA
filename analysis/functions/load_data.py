...
def load_dataframes(folder_list, basepath, prefix, folder_dataset, load_sc_features=True):
    ...
    lbl_conv_obj = label_converter.LabelConverter(
        path_preload=os.path.join(basepath, folder_list[0], 'class_conversion.csv'))
    ...
    for f in folder_list:
        ...
        for entity, patients in pkl_obj.items():
            for pat_path, pat_data_packed in patients.items():
                ...
                pat_entropy = entropy(
                    pat_prediction_vector, base=len(pat_prediction_vector))
    ...
    columns_df = ['ID', 'pat_path', 'fold', 'gt_label', 'pred_lbl',
                  'MIL loss', 'entropy', 'quality_category', 'myb_annotated']
    ...
    for row_idx in tqdm(range(len(patient_dataframe))):
        ...
        pat_sc_dataframe['im_path'] = [os.path.join(
            pat_path, generate_tiffname(x)) for x in range(num_single_cells)]
        ...
        if load_sc_features:
            pat_features = np.load(os.path.join(
                pat_path, 'processed/{}bn_features_layer_7.npy'.format(prefix)))
            ...
            pat_sc_feature_dataframe = pd.DataFrame(pat_features_flattened, columns=[
    ...
    whole_sc_dataframe = whole_sc_dataframe.join(
        mll_annotation_dataframe, on=['ID', 'im_tiffname'], how='left').set_index('ID')
