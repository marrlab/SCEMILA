...
def plot(patient_dataframe, true_list, save_in=None):
    ...
    fpr, tpr, _ = metrics.roc_curve(df_packed['is_true'], df_packed['percentage'])
    ...
    prec, rec, _ = metrics.precision_recall_curve(df_packed['is_true'], df_packed['percentage'])
    ...
    ax.text(0.1, 0.1, "AUC: {:.3f}".format(metrics.auc(rec, prec)), fontsize=12)
    ax.tick_params(axis='both', which='major', labelsize=12)

    plt.show()
    if not save_in is None:
