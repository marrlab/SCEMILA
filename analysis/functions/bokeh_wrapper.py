...
def export_swarmplot(df, xlim, ylim, title, highlight_idx=None, path_save=None, plot_quantiles=None, **kwargs):
    ...
    for ctype in legend_order:
        ...
        if len(ctype_df) > 0:
            ax.scatter(x=ctype_df.x, y=ctype_df.y, color=ctype_df.color, edgecolor=ctype_df.edgecolor, 
                               label=font_matching.edit(capitalize(ctype)), marker=shape_get_matplotlib(ctype), s=dotsize, linewidth=0.5, **kwargs)
    ...
    if not plot_quantiles is None:
        quant_distribution, borders = calculate_cells_in_quantiles(df, 
                                                       plot_quantiles, 
...
def calculate_cells_in_quantiles(df, target_column, quantiles=[0.25, 0.5, 0.75], percent_columns=True, 
                                sort_by_percentage=True, group_index=True, round_data=True):
...
def plot_piechart(data_with_mappings_and_coordinates, att_column, scale_factor=1, path_save=None, group_pie=True):
    quant_distribution, borders = calculate_cells_in_quantiles(data_with_mappings_and_coordinates, 
                                                       att_column, 
                                                       group_index=group_pie, round_data=False)
...
def export_umap(df_in, minimalize=True, title='UMAP embedding: Predicted single cell class', data_column='mll_annotation', 
               ...
               grayscatter=True, dotsize=35, path_save=None, custom_cmap=cm.jet, thresh_sol=0.0):
...
#         ax.scatter(df_grayscatter.x, df_grayscatter.y, color='whitesmoke', edgecolor='whitesmoke', s=df_grayscatter.dotsize)
    ...
    if(data_column == 'mll_annotation'):
        ...
        for label in classes_order:
            ...
            sc = ax.scatter(df_plot_tmp.x, df_plot_tmp.y, color=col_get(label), edgecolor=col_edge_get(label),
                            s=dotsize, label=font_matching.edit(label_cap), linewidth=0.5,
    ...
    if ('solitary' in data_column):
        norm = mpt_colors.Normalize(vmin=df_in[data_column].min(), vmax=df_in[data_column].max())
        cmap = custom_cmap
    ...
    if (data_column == 's_full_plot'):
        ...
        get_sol_color = lambda x: col_dict[x]['col']
        ...
        df_in['solitary_softmax_color'] = df_in['solitary_softmax_idxmax'].apply(get_sol_color)

        # filter all data points, where the raw attention is below a certain percentile for that column
        df_tmp = []
        for c in col_dict.keys():
            att_raw_column = 'att_raw_' + c[17:]
            
            # drop all rows, where raw attentino value is below the quantile val and solitary_softmax_max = c
            df_filtered = df_in.loc[df_in['solitary_softmax_idxmax'] == c]
                 
            df_filtered['solitary_softmax_alpha'] = (df_filtered[att_raw_column] - df_filtered[att_raw_column].min())/(df_filtered[att_raw_column].max()-df_filtered[att_raw_column].min())
            quantile_val = df_filtered[att_raw_column].quantile(thresh_sol)
            df_filtered = df_filtered.loc[df_filtered[att_raw_column] > quantile_val]
        ...
        df_in['solitary_softmax_color'] = df_in.apply(lambda x: color_transform(x.solitary_softmax_color, x.solitary_softmax_alpha), axis=1)
        ...
        for key, val in col_dict.items():
            ax.scatter(-100, -100, c=val['col'], label=val['display'], s=dotsize)
    
        # iterate over all columns, only 
        sc = ax.scatter(df_in.x, df_in.y, c=df_in['solitary_softmax_color'],s=dotsize)
...
def sol_clusterplot(df_in, path_save=None):
    ...
    entities_available = ['AML-PML-RARA', 'AML-NPM1', 'AML-CBFB-MYH11', 'AML-RUNX1-RUNX1T1', 'SCD']
    for entity in entities_available:
        patient_df_f0_ent = df_in.loc[df_in['gt_label'] == entity]
        clusterplot_structure[entity] = patient_df_f0_ent.sort_values(by='mil_prediction_{}'.format(entity), ascending=False).index.unique()
    ...
    fig, ax = plt.subplots(max_len, 6, figsize=(12, max_len/4), constrained_layout=False)
    ...
    for entity in entities_available:
        ...
        row_counter = 0
        for patient_number in range(min(len(clusterplot_structure[entity]),max_len)):
            patient = clusterplot_structure[entity][row_counter]
            sol_att_bar_plot(df_in.loc[patient], ax[row_counter, column_counter])
            if(row_counter == 0):
...
def sol_att_bar_plot(data, ax):
    ...
    col_transform = lambda x: CLASS_COLORS[x][0]
    order_transform = lambda x: CLASS_COLORS[x][1]
    ...
    data['plot_order'] = data[CLASS_COLORS.keys()].idxmax(axis=1).apply(order_transform)
    data = data.sort_values(by=att_col, ascending=True)    
    
    # add solitary prediction results
    data['solitary_color'] = data[CLASS_COLORS.keys()].idxmax(axis=1).apply(col_transform)
    ...
    # plot plot    
    ax.barh(0, width=buffer_width, left=buffer_left, height=0.7, color=data['solitary_color'])
